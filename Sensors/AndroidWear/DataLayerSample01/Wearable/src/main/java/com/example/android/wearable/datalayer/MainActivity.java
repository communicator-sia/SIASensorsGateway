...
public class MainActivity extends Activity implements ConnectionCallbacks,
    ...
    public void sendAccelData(Float x, Float y, Float z, long timestamp){
        PutDataMapRequest putDataMapRequest = PutDataMapRequest.create("/accel");
        ...
        Wearable.DataApi.putDataItem(mGoogleApiClient, request).setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
            ...
            public void onResult(DataApi.DataItemResult dataItemResult) {
                if (!dataItemResult.getStatus().isSuccess()) {
                    System.out.println("Failed to send accel data ");
                } else {
                    System.out.println("Successfully sent accel data");
                }
            }
        });
    }

    public void sendGyroData(Float gx, Float gy, Float gz, long timestamp){
        PutDataMapRequest putDataMapRequest = PutDataMapRequest.create("/gyro");
        putDataMapRequest.getDataMap().putFloat("gx", gx);
        putDataMapRequest.getDataMap().putFloat("gy", gy);
        putDataMapRequest.getDataMap().putFloat("gz", gz);
        putDataMapRequest.getDataMap().putLong("timestamp", timestamp);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            sendAccelData(event.values[0], event.values[1], event.values[2], timestamp);
            detectShake(event);
        }
        else if(event.sensor.getType() == Sensor.TYPE_GYROSCOPE) {
            long timestamp = new Date().getTime();
            sendGyroData(event.values[0], event.values[1], event.values[2], timestamp);
    ...
    private void detectShake(SensorEvent event) {
        ...
        if((now - mShakeTime) > SHAKE_WAIT_TIME_MS) {
            ...
            float gForce = (float) Math.sqrt(Float.valueOf(gX * gX + gY * gY + gZ * gZ));
